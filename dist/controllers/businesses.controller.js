...
class BusinessesController {
    ...
    static async getBusinesses(req, res) {
        try {
            ...
            return (0, response_1.sendSuccess)(res, { businesses: formattedBusinesses }, "Businesses fetched successfully");
        }
        catch (error) {
    ...
    static async createBusiness(req, res) {
        try {
            ...
            if (!name ||
                ...
                !sector ||
                !category) {
                return (0, response_1.sendError)(res, 400, "All required fields must be provided");
            }
            // Validate MOF Registration Number if MOF Registration is true
            if (mofRegistration && !mofRegistrationNumber?.trim()) {
                return (0, response_1.sendError)(res, 400, "MOF Registration Number is required when MOF Registration is selected");
            }
            // Validate SSM number format (should be 12 digits)
            const ssmClean = ssm.replace(/[-\s]/g, "");
            if (!/^\d{12}$/.test(ssmClean)) {
                return (0, response_1.sendError)(res, 400, "Invalid SSM number format (12 digits required)");
            }
            // Validate phone number format
            const phoneClean = phone.replace(/[-\s]/g, "");
            if (!/^(\+?60|0)[0-9]{8,11}$/.test(phoneClean)) {
                return (0, response_1.sendError)(res, 400, "Invalid phone number format");
            }
            // Validate URL if provided
            if (url && url.trim()) {
                const urlPattern = /^https?:\/\/.+\..+/;
                if (!urlPattern.test(url)) {
                    return (0, response_1.sendError)(res, 400, "Invalid URL format");
                }
            }
            // Check if SSM number already exists
            const existingBusiness = await (0, database_1.db)("businesses")
                .where("b_ssm_number", ssmClean)
                .first();
            if (existingBusiness) {
                return (0, response_1.sendError)(res, 409, "Business with this SSM number already exists");
            }
            ...
            // Insert the business
            const [businessId] = await (0, database_1.db)("businesses").insert(businessData);
            // Fetch the created business with proper field mapping
            const createdBusiness = await (0, database_1.db)("businesses")
                .select("b_id as id", "b_company_name as name", "b_ssm_number as ssm", "b_office_number as phone", "b_address as address", "b_type as type", "b_sector as sector", "b_category as category", "b_mof_registration as mofRegistration", "b_mof_registration_number as mofRegistrationNumber", "b_url as url", "b_created_at as createdAt", "b_modified_at as modifiedAt")
            ...
            return (0, response_1.sendSuccess)(res, { business: formattedBusiness }, "Business created successfully");
        }
        catch (error) {
    ...
    static async updateBusiness(req, res) {
        try {
            ...
            if (!businessId || isNaN(Number(businessId))) {
                return (0, response_1.sendError)(res, 400, "Invalid business ID");
            }
            // Get user ID from request
            const userId = req.user?.id;
            if (!userId) {
            ...
            const existingBusiness = await (0, database_1.db)("businesses")
                ...
                .first();
            if (!existingBusiness) {
                return (0, response_1.sendError)(res, 404, "Business not found or access denied");
            }
            // Basic validation
            if (!name ||
                !ssm ||
            ...
            const duplicateSSM = await (0, database_1.db)("businesses")
                .where("b_ssm_number", ssmClean)
                .where("b_id", "!=", businessId)
                .where("b_status", 1)
                .first();
            ...
            // Update the business
            await (0, database_1.db)("businesses")
                .where("b_id", businessId)
                .where("b_user_id", userId)
                .update(businessData);
            // Fetch the updated business with proper field mapping
            const updatedBusiness = await (0, database_1.db)("businesses")
                .select("b_id as id", "b_company_name as name", "b_ssm_number as ssm", "b_office_number as phone", "b_address as address", "b_type as type", "b_sector as sector", "b_category as category", "b_mof_registration as mofRegistration", "b_mof_registration_number as mofRegistrationNumber", "b_url as url", "b_created_at as createdAt", "b_modified_at as modifiedAt")
                .where("b_id", businessId)
                .first();
            // Convert mofRegistered to boolean
            ...
            return (0, response_1.sendSuccess)(res, { business: formattedBusiness }, "Business updated successfully");
        }
        catch (error) {
    ...
    static async deleteBusiness(req, res) {
        try {
            ...
            return (0, response_1.sendSuccess)(res, { businessId: Number(businessId) }, "Business deleted successfully");
        }
        catch (error) {
    ...
    static async getBusinessById(req, res) {
        try {
            ...
            // Fetch the business
            const business = await (0, database_1.db)("businesses")
                .select("b_id as id", "b_company_name as name", "b_ssm_number as ssm", "b_office_number as phone", "b_address as address", "b_type as type", "b_sector as sector", "b_category as category", "b_mof_registration as mofRegistration", "b_mof_registration_number as mofRegistrationNumber", "b_url as url", "b_created_at as createdAt", "b_modified_at as modifiedAt")
                .where("b_id", businessId)
                .where("b_user_id", userId)
                .where("b_status", 1)
            ...
            return (0, response_1.sendSuccess)(res, { business: formattedBusiness }, "Business fetched successfully");
        }
        catch (error) {
