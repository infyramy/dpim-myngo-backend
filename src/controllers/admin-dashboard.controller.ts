  ...
  static async getStateOverview(_req: Request, res: Response) {
    try {
      ...
      return sendSuccess(
        ...
        "State overview data retrieved successfully"
      );
    } catch (error) {
      ...
      const dashboardData = {
        stats: {
          ...
          totalPendingApplications: Number(
            totalPendingApplications?.count || 0
          ),
        },
        stateData: stateOverview,
      };
