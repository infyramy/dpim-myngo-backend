    ...
    try {
      ...
      const businesses = await db("businesses")
        .select(
          ...
          "b_ssm_number as ssm",
          "b_office_number as phone",
          "b_address as address",
          "b_type as type",
          "b_sector as sector",
      ...
      // Convert mofRegistered to boolean and parse images JSON for each business
    ...
    try {
      ...
      if (!/^\d{12}$/.test(ssmClean)) {
        return sendError(
          res,
          400,
          "Invalid SSM number format (12 digits required)"
        );
      }
      ...
      if (existingBusiness) {
        return sendError(
          res,
          409,
          "Business with this SSM number already exists"
        );
      }
