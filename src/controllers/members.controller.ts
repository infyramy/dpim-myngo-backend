...
export class MembersController {
    ...
    try {
      ...
      let query = db("user as u")
        ...
        .select(
          ...
          "u.user_mobile_number as phone",
          "u.user_status as status",
          "u.user_created_at as joinDate",
          db.raw("COUNT(DISTINCT p.p_id) as productsCount"),
          db.raw("COUNT(DISTINCT b.b_id) as businessesCount")
        )
        .leftJoin("products as p", function () {
          this.on("p.p_business_id", "=", "b.b_id").andOn(
            "p.p_status",
            "=",
            db.raw("1")
          );
      ...
      if (search) {
        ...
        query = query.where(function () {
          this.where("u.user_fullname", "like", searchTerm).orWhere(
            "u.user_email",
            "like",
            searchTerm
          );
    ...
    try {
      ...
      const member = await db("user as u")
        .select(
          ...
          "u.user_email as email",
          "u.user_mobile_number as phone",
          "u.user_residential_address as address",
          "u.user_postcode as postcode",
          "u.user_city as city",
          "u.user_state as state",
          "u.user_status as status",
          "u.user_created_at as joinDate",
          "u.user_last_logged_in as lastLogin",
          db.raw("COUNT(DISTINCT b.b_id) as businessesCount"),
          db.raw("COUNT(DISTINCT p.p_id) as productsCount")
        )
        .leftJoin("businesses as b", function () {
          this.on("b.b_user_id", "=", "u.user_id").andOn(
            "b.b_status",
            "=",
            db.raw("1")
          );
      ...
      const products = await db("products as p")
        ...
        .select(
          ...
          "b.b_company_name as businessName"
        )
   ...
   */
  static async reactivateMember(req: Request, res: Response) {
    try {
      const { id } = req.params;
    ...
    try {
      ...
      const stats = await db("user")
        .select(
          ...
          db.raw("SUM(CASE WHEN user_status = 1 THEN 1 ELSE 0 END) as active"),
          db.raw(
            "SUM(CASE WHEN user_status = 3 THEN 1 ELSE 0 END) as suspended"
          ),
          db.raw("SUM(CASE WHEN user_status = 2 THEN 1 ELSE 0 END) as pending")
        )
        .where("user_role", "user")
